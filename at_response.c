...
static int at_response_cereg (struct pvt* pvt, char* str, size_t len)
{
	...
	if (at_parse_cereg (str, len, &d, &pvt->lte_reg_status, &lac, &ci))
	{
	...
	if (d)
	{
		pvt->lte_registered = 1;
		manager_event_device_status(PVT_ID(pvt), "LTE Register");
	}
	else
	{
		pvt->lte_registered = 0;
		manager_event_device_status(PVT_ID(pvt), "LTE Unregister");
	}
