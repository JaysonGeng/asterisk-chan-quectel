...
static char* cli_show_device_state (struct ast_cli_entry* e, int cmd, struct ast_cli_args* a)
{
	...
	if (pvt)
	{
		...
		ast_cli (a->fd, "  LTE Registration Status : %s\n", LTE_regstate2str(pvt->lte_reg_status));
...
static char* cli_cmd (struct ast_cli_entry* e, int cmd, struct ast_cli_args* a)
{
	...
	switch (cmd)
	{
		case CLI_INIT:
			...
			e->usage =
				"Usage: quectel cmd <device> <command>\n"
				"       Send the <command> to the device. New line automatically added.\n";
			return NULL;
	...
	if (pvt)
	{
		at_enqueue_user_cmd(&pvt->sys_chan, a->argv[3]);
		res = CLI_SUCCESS;
		ast_mutex_unlock (&pvt->lock);
	}
	else
...
static char* cli_cereg (struct ast_cli_entry* e, int cmd, struct ast_cli_args* a)
{
	pvt_t* pvt;
	char* res = CLI_FAILURE;
	...
	{
		case CLI_INIT:
			e->command = "quectel cereg";
			e->usage =
				"Usage: quectel cereg <device>\n"
				"       Send AT+CEREG? command to query LTE registration status.\n";
			return NULL;
...
static struct ast_cli_entry cli[] = {
	...
	AST_CLI_DEFINE (cli_cereg,		"Send AT+CEREG? command to query LTE registration status"),
