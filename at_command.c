...
EXPORT_DEF int at_enqueue_initialization(struct cpvt *cpvt, at_cmd_t from_command)
{
	...
	static const char cmd16a[] = "AT+CEREG=2\r";
	static const char cmd16b[] = "AT+CEREG?\r";
...
//	static const char cmd18[] = "AT+CLIP=0\r";
	...
	static const at_queue_cmd_t st_cmds[] = {
		...
		ATQ_CMD_DECLARE_STI(CMD_AT_CEREG_INIT,cmd16a),	/* LTE registration status setting */
		ATQ_CMD_DECLARE_ST(CMD_AT_CEREG, cmd16b),	/* LTE registration status */
...
/*!
 * \brief Enqueue the AT+CEREG? command
 * \param cpvt -- cpvt structure
 * \return 0 on success
 */

EXPORT_DEF int at_enqueue_cereg(struct cpvt *cpvt)
{
	static const char cmd[] = "AT+CEREG?\r";
	static at_queue_cmd_t at_cmd = ATQ_CMD_DECLARE_ST(CMD_AT_CEREG, cmd);
