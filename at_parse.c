...
/*!
 * \brief Parse a CEREG response
 * \param str -- string to parse (null terminated)
 * \param len -- string lenght
 * \param lte_reg -- a pointer to a int
 * \param lte_reg_status -- a pointer to a int
 * \param lac -- a pointer to a char pointer
 * \param ci -- a pointer to a char pointer
 * @note str will be modified when the CEREG message is parsed
 * \retval  0 success
 * \retval -1 parse error
 */

EXPORT_DEF int at_parse_cereg (char* str, unsigned len, int* lte_reg, int* lte_reg_status, char** lac, char** ci)
{
	unsigned	i;
	int	state;
	char*	p1 = NULL;
	...
	*ci  = NULL;
	...
	/*
	 * parse CEREG response in the following format:
	 * +CEREG: [<p1>,]<p2>[,<p3>,<p4>]
	 */
	...
	{
		errno = 0;
		*lte_reg_status = (int) strtol (p1, (char**) NULL, 10);
		if (*lte_reg_status == 0 && errno == EINVAL)
		{
			*lte_reg_status = -1;
			return -1;
		}

		if (*lte_reg_status == 1 || *lte_reg_status == 5)
		{
			*lte_reg = 1;
		}
	}
